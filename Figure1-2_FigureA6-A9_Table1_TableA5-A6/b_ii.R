...
compare = function(n = 500, p = 20, it = 50, a1 = 0.05, a2 = 0.2){
  ...
  para = NULL
  while(iter <= it) {
    sam = sample.sim(n = n, p = p, a1 = a1, a2 = a2, noise = "ii", outlier = "b", seed = iter + 150)
    x   = sam$x
    y   = sam$y
    datay[, iter] = y
    datax = cbind(datax, x)
  ...
  write.table(datax, file= paste("sim_ii_b/datax", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(datay, file= paste("sim_ii_b/datay", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(beta, file= paste("sim_ii_b/beta", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
...
sse.pert = read.table("../pert/sse_pert_ii_b.txt")
coe.pert = read.table("../pert/coe_pert_ii_b.txt")
...
ii_b1 = ggplot(data = dat, aes(x = Proportion.outlier, y = SSE)) + geom_boxplot(aes(fill = Method), position=position_dodge(1)) + labs(x = "Proportion of outliers", y = "log2(SSE)") + scale_x_discrete(breaks = NULL) +
...
library(cowplot)
library(PASWR)
ii_b2 = plot_grid(res4$fig, res5$fig, res1$fig, res2$fig, ncol = 2)

save(ii_b2, file = "ii_b2.RData")
