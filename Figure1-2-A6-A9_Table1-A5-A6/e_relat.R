...
compare = function(n = 500, p = 20, it = 50, a1 = 0.05, a2 = 0.2){
  ...
  para = NULL
  while(iter <= it) {
    sam = erelated.sim(n = n, p = p, a1 = a1, a2 = a2, seed = iter)
    x   = sam$x
    y   = sam$y
    datay[, iter] = y
    datax = cbind(datax, x)
  ...
  write.table(datax, file= paste("sim_e_relate/datax", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(datay, file= paste("sim_e_relate/datay", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(beta, file= paste("sim_e_relate/beta", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  result = list(tpr = tpr.fardeep, fpr = fpr.fardeep, k = para, sse.ciber = sse.ciber, sse.fardeep = sse.fardeep, sse.nnls = sse.nnls, sse.dcq = sse.dcq,
                ...
                coe.dcq = coe.dcq, coe.nnls = coe.nnls, coe = beta)
  return(result)
}
...
sse.pert = read.table("../pert/sse_pert_e_relate.txt")
coe.pert = read.table("../pert/coe_pert_e_relate.txt")
...
e.p1 = ggplot(data = dat, aes(x = Proportion.outlier, y = SSE)) + geom_boxplot(aes(fill = Method), position=position_dodge(1)) + labs(x = "Proportion of outliers", y = "log2(SSE)") + scale_x_discrete(breaks = NULL) +
  facet_wrap( ~ Proportion.outlier, scales="free") + coord_cartesian(ylim=c(-8, 11)) + theme_gray() + scale_fill_manual(values=colo) +
...
library(cowplot)
library(PASWR)
e.p2 = plot_grid(res4$fig, res5$fig, res1$fig, res2$fig, ncol = 2)

save(e.p2, file = "e.p2.RData")
