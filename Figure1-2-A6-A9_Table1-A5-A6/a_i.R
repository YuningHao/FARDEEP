...
compare = function(n = 500, p = 20, it = 50, a1 = 0.05, a2 = 0.2){
  ...
  para = NULL
  while(iter <= it) {
    sam = sample.sim(n = n, p = p, a1 = a1, a2 = a2, noise = "i", outlier = "a", seed = iter)
    x   = sam$x
    y   = sam$y
    datay[, iter] = y
    datax = cbind(datax, x)
  ...
  write.table(datax, file= paste("sim_i_a/datax", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(datay, file= paste("sim_i_a/datay", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
  write.table(beta, file= paste("sim_i_a/beta", a1, ".txt", sep = ""), row.names=F, col.names=F, quote=F)
...
sse.pert = read.table("../pert/sse_pert_i_a.txt")
coe.pert = read.table("../pert/coe_pert_i_a.txt")
...
i_a1 = ggplot(data = dat, aes(x = Proportion.outlier, y = SSE)) + geom_boxplot(aes(fill = Method), position=position_dodge(1)) + labs(x = "Proportion of outliers", y = "log2(SSE)") + scale_x_discrete(breaks = NULL) +
...
library(cowplot)
library(PASWR)
i_a2 = plot_grid(res4$fig, res5$fig, res1$fig, res2$fig, ncol = 2)

save(i_a2, file = "i_a2.RData")
